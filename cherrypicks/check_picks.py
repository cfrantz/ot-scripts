...
def prs_by_merge_date(table):
    def get_merge_date(pr):
        return datetime.datetime.fromisoformat(table[pr]["mergedAt"])
    return sorted(table.keys(), key=get_merge_date)
...
def render_table(table):
    ...
    total = len(table)
    mapping = {}
    for k in prs_by_merge_date(table):
        v = table[k]
...
def compute_moves(initial, final_map):
    """
    Start from an initial array, goal is to reach target array given as mapping from value to position.
    """
    final = {
        final_map[x]: x for x in initial
    }
    moves = []
    pos = 0
    # This is really inefficient, we don't try to be smart here.
    current = initial
    while pos < len(current):
        if current[pos] != final[pos]:
            # Find which element should be here.
            other_pos = current.index(final[pos])
            # Move it.
            moves.append((other_pos, pos))
            # Apply change to `current`.
            assert pos < other_pos
            current = current[:pos] + [current[other_pos]] + current[pos:other_pos] + current[other_pos+1:]
        else:
            pos += 1
    return moves
...
def unittest_moves():
    ...
    print(compute_moves(initial, final_map))
...
def reorder_rows(sheet, first_table_row, table):
    sheet_prs = [int(pr) for pr in sheet.col_values(1)[first_table_row:]]
    ...
    requests = [{
            "moveDimension": {
                "source": {
                    "sheetId": sheet.id,
                    "dimension": "ROWS",
                    "startIndex": first_table_row + from_idx,
                    "endIndex": first_table_row + from_idx + 1,
                },
                "destinationIndex": first_table_row + to_idx,
            }
        }
    ...
    # Re-query after the move to make sure that it is correct.
    sheet_prs = [int(pr) for pr in sheet.col_values(1)[first_table_row:]]
    for (idx, pr) in enumerate(sheet_prs):
        assert table["mapping"][pr] == idx, "The order of the rows after the move does not match the expected order, this might indicate a bug in the tool, please revert the change on the spreadsheet and fix the bug"
...
def render_gspread(spreadsheet, table):
    ...
    if cookie["first_table_row"] != first_table_row:
        sys.exit("Error: the spreadsheet current uses first_table_row={} but this script wants {}.\n".format(cookie["first_table_row"], first_table_row) +
    ...
    # We might have to reorder the existing spreadsheet rows to match the order we have
    reorder_rows(sheet, first_table_row, table)
