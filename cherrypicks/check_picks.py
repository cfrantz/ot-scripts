...
def render_gspread(spreadsheet, table):
    ...
    sheet.batch_format([
        {
            "range": '{}'.format(first_table_row + i),
            "format": {
                "backgroundColor": COLORS_GSPREAD[row["color"]]
            },
        }
        for (i, row) in enumerate(table["rows"]) if row["color"] is not None
    ])
